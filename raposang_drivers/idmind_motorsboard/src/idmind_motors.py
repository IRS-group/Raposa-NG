...
import rospy
from idmind_sensorsboard.msg import Log
...
class MotorBoard(object):
    ...
    def __init__(self, baudrate=57600, timeout=.5):
        #####################
        #  BOARD VARIABLES  #
        #####################
        ...
        try:
        	...
        	self.ser = IDMindSerial(port, baudrate=baudrate, timeout=timeout)
    ...
    def get_velocity_motor_ticks(self):
        ...
        try:
            ...
            bytesum = reduce(lambda x, y: x + y, [ord(chr(el)) for el in res[:-2]])

            if ord(chr(res[0])) != GET_ENC or chk != (bytesum & 0xffff):
                return False
            else:
    ...
    def get_arm_position(self):
        ...
        try:
            ...
            if ord(chr(res[0])) != msg[0] or chk != (bytesum & 0xffff):
                self.log("Wrong message: {}".format(res), 3)
                return False
            else:
    ...
    def set_motor_velocities_control(self):
        ...
        if dt > self.time_tolerance:
            ...
            msg = [SET_VEL]
            ratio = old_div(self.max_ref, self.max_linear_speed)
            ...
            try:
                r = min(1, old_div(self.max_ref,max([abs(a) for a in refs])))
    ...
    def publish_data(self):
        ...
        try:
            if not self.get_velocity_motor_ticks():
                ...
                dist_msg.front_left = old_div(per * self.ticks["front_left"],self.ticks_turn["front_left"])
                dist_msg.front_right = old_div(per * self.ticks["front_right"],self.ticks_turn["front_right"])
                if self.kinematics == "omni":
                    dist_msg.back_left = old_div(per * self.ticks["back_left"],self.ticks_turn["back_left"])
                    dist_msg.back_right = old_div(per * self.ticks["back_right"],self.ticks_turn["back_right"])
...
if __name__ == "__main__":
    ...
    m = MotorBoard(baudrate=57600, timeout=0.5)
    m.start()
