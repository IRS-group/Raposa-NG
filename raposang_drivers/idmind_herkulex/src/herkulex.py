...
class servo(object):
    ...
    def get_servo_position(self):
        ...
        try:
            ...
            if (self.servomodel==0x06) or (self.servomodel == 0x04):
                return ((ord(chr(rxdata[10]))&0xff)<<8) | (ord(chr(rxdata[9]))&0xFF)
            else:
                #print ord(chr(rxdata[9])),ord(chr(rxdata[10]))
                return ((ord(chr(rxdata[10]))&0x03)<<8) | (ord(chr(rxdata[9]))&0xFF)
    ...
    def get_servo_torque(self):
        ...
        try:
            rxdata = SERPORT.read(13)
            if ord(chr(rxdata[10]))<=127:
                return ((ord(chr(rxdata[10]))&0x03)<<8) | (ord(chr(rxdata[9]))&0xFF)
            else:
                return (ord(chr(rxdata[10]))-0xFF)*0xFF + (ord(chr(rxdata[9]))&0xFF)-0xFF
        except HerkulexError:
            raise HerkulexError("could not communicate with motors")
    ...
    def get_position_p(self):
        ...
        try:
            rxdata = SERPORT.read(13)
            return (ord(chr(rxdata[10]))*256)+(ord(chr(rxdata[9]))&0xff)
        except HerkulexError:
            raise HerkulexError("could not communicate with motors")
