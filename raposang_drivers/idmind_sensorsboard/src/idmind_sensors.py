...
class SensorBoard:
    ...
    def __init__(self, address="/dev/idmind-sensorsboard", baudrate=57600, timeout=.5):
        ...
        if rospy.has_param("/idmind_sensors/board"):
            address = rospy.get_param("/idmind_sensors/board")
            self.log(address, 1, alert="warn")
    ...
    def get_voltages(self):
        ...
        try:
            ...
            if ord(chr(res[0])) != GET_VOLTAGES or chk != (bytesum & 0xffff):
                return False
            else:
                self.voltages["motor_battery"] = ord(chr(res[1])) * 0.1
                self.voltages["electronic_battery"] = ord(chr(res[2])) * 0.1
                self.voltages["cable"] = ord(chr(res[3])) * 0.1
                self.relays["motor_relay"] = (ord(chr(res[4])) & MOTOR_BIT) == 1
                self.relays["cable_relay"] = ((ord(chr(res[4])) & CABLE_BIT) >> 1) == 1
                self.relays["electronic_relay"] = ((ord(chr(res[4])) & ELECTRONIC_BIT) >> 2) == 1
                return True
        except Exception as e:
    ...
    def publish_data(self):
        ...
        try:
            if not self.get_voltages():
                ...
                if (v_msg.motor_battery < MIN_VOL or v_msg.electronic_battery < MIN_VOL) and not v_msg.cable_relay:
